...
begin
  ...
  try
    try
      scoDados.Params.Values['DATABASE'] := Config.ReadString('SSBasico', 'DATABASE', '');
      scoDados.Params.Values['USER_NAME'] := Config.ReadString('SSBasico', 'USERNAME', '');
      scoDados.Params.Values['PASSWORD'] := Decoder64.DecodeString(Config.ReadString('SSBasico', 'PASSWORD', ''));
...
//                               'Servidor : ' + Conexao.Params.Values['HostName'] + #13 +
//                               'DriverName : ' + Conexao.Params.Values['drivername']);
      end;
    end;
  finally
    FreeAndNil(Config);
