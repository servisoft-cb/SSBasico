...
type
  TfHistGeral = class(TForm)
    ...
    qVendedorCodVendedor2: TIntegerField;
    ...
    qDevolucaoDtEntrada: TDateField;
    qDevolucaoVlrDevolucao: TFloatField;
  ...
  private
    ...
    procedure Monta_SQLDevolucao;
    procedure Monta_SQLDevolucao2(Tipo : Integer);
    ...
    procedure Monta_CodAgrupadoDev;
  public
...
procedure TfHistGeral.Monta_CodAgrupadoDev;
begin
  vGera := True;
  if DM1.tCliente.Locate('CGCCPF',qDevolucao2.FieldByName('CGC').AsString,[loCaseInsensitive]) then
    if (RxDBLookupCombo5.Text <> '') and (DM1.tClienteCodGrupo.AsInteger <> RxDBLookupCombo5.KeyValue) then
      vGera := False;
  if vGera then
    begin
      if (qDevolucao2.FieldByName('CGC').IsNull) or (qDevolucao2.FieldByName('CGC').AsString = '  .  .   /    -  ') then
        vCodAgrupado := 'F' + qDevolucao2.FieldByName('CodForn').AsString
      else
      if DM1.tCliente.Locate('CGCCPF',qDevolucao2.FieldByName('CGC').AsString,[loCaseInsensitive]) then
        begin
          if (CheckBox3.Checked) and (DM1.tClienteCodGrupo.AsInteger > 0) then
            vCodAgrupado := 'G' + DM1.tClienteCodGrupo.AsString
          else
            vCodAgrupado := 'C' + DM1.tClienteCodigo.AsString;
        end;
    end;
end;

procedure TfHistGeral.Monta_SQLDevolucao2(Tipo : Integer);
begin
  qDevolucao2.Close;
  qDevolucao2.SQL.Clear;
  qDevolucao2.SQL.Add('SELECT Dbfornecedores.NomeForn, SUM( DbnentradaItens.VlrTotalItens ) VlrDevolucao, Dbfornecedores.CodForn, Dbfornecedores.CGC, SUM( dbNEntradaItens.Qtd ) Qtd ');
  if Tipo = 1 then
    qDevolucao2.SQL.Add(' , DbProduto.Nome NomeProduto, dbProduto.Codigo CodProduto, dbProduto.Referencia');
  qDevolucao2.SQL.Add('FROM "dbNEntrada.DB" Dbnentrada');
  qDevolucao2.SQL.Add('   INNER JOIN "dbNEntradaItens.DB" Dbnentradaitens');
  qDevolucao2.SQL.Add('   ON  (Dbnentrada.Filial = Dbnentradaitens.Filial)');
  qDevolucao2.SQL.Add('   AND  (Dbnentrada.NumNEntr = Dbnentradaitens.NumNEntr)');
  qDevolucao2.SQL.Add('   AND  (Dbnentrada.CodForn = Dbnentradaitens.CodForn)');
  qDevolucao2.SQL.Add('  INNER JOIN "dbFornecedores.DB" Dbfornecedores');
  qDevolucao2.SQL.Add('   ON  (Dbnentrada.CodForn = Dbfornecedores.CodForn)');
  if Tipo = 1 then
    begin
      qDevolucao2.SQL.Add('  INNER JOIN "dbProduto.DB" DbProduto');
      qDevolucao2.SQL.Add('   ON  (DbNEntradaItens.CodMaterial = dbProduto.Codigo)');
    end;
  qDevolucao2.SQL.Add('WHERE  Dbnentrada.VlrDevolucao > 0');
  if not CheckBox4.Checked then
    qDevolucao2.SQL.Add('  AND  (dbNEntradaItens.MatProd = ''P'')');
  if RxDBLookupCombo2.Text <> '' then
    begin
      qDevolucao2.SQL.Add('  AND (dbNEntrada.Filial = :Filial)');
      qDevolucao2.ParamByName('Filial').AsInteger := RxDBLookupCombo2.KeyValue;
    end;
  if RxDBLookupCombo1.Text <> '' then
    begin
      if DM1.tCliente.Locate('Codigo',RxDBLookupCombo1.KeyValue,[loCaseInsensitive]) then
        begin
          qDevolucao2.SQL.Add('  AND (dbFornecedores.CGC = :CGC)');
          qDevolucao2.ParamByName('CGC').AsString := DM1.tClienteCgcCpf.AsString;
        end;
    end;
  if DateEdit1.Date > 0 then
    begin
      qDevolucao2.SQL.Add('  AND (dbNEntrada.DtEntrada >= :DtEntradaIni)');
      qDevolucao2.ParamByName('DtEntradaIni').AsDate := DateEdit1.Date;
    end;
  if DateEdit2.Date > 0 then
    begin
      qDevolucao2.SQL.Add('  AND (dbNEntrada.DtEntrada <= :DtEntradaFin)');
      qDevolucao2.ParamByName('DtEntradaFin').AsDate := DateEdit2.Date;
    end;
  if RxDBLookupCombo3.Text <> '' then
    begin
      qDevolucao2.SQL.Add('  AND (dbNEntradaItens.CodMaterial = :CodMaterial)');
      qDevolucao2.ParamByName('CodMaterial').AsInteger := RxDBLookupCombo3.KeyValue;
    end;
  qDevolucao2.SQL.Add('GROUP BY Dbfornecedores.NomeForn, Dbfornecedores.CodForn, Dbfornecedores.CGC ');
  if Tipo = 1 then
    qDevolucao2.SQL.Add(' , DbProduto.Nome, dbProduto.Codigo, dbProduto.Referencia');
  qDevolucao2.Open;
end;
...
begin
  ...
  if RxDBLookupCombo4.Text <> '' then
    begin
      qPedidoCliProd.SQL.Add('  AND ((dbPedido.CodVendedor = :CodVendedor) or (dbPedido.CodVendedor2 = :CodVendedor2))');
      qPedidoCliProd.ParamByName('CodVendedor').AsInteger  := RxDBLookupCombo4.KeyValue;
      qPedidoCliProd.ParamByName('CodVendedor2').AsInteger := RxDBLookupCombo4.KeyValue;
    end;
  if DateEdit1.Date > 0 then
    begin
...
begin
  ...
  if RxDBLookupCombo4.Text <> '' then
    begin
      qPedidoCliProdMes.SQL.Add('  AND ((dbPedido.CodVendedor = :CodVendedor) or (dbPedido.CodVendedor2 = :CodVendedor2))');
      qPedidoCliProdMes.ParamByName('CodVendedor').AsInteger  := RxDBLookupCombo4.KeyValue;
      qPedidoCliProdMes.ParamByName('CodVendedor2').AsInteger := RxDBLookupCombo4.KeyValue;
    end;
  if DateEdit1.Date > 0 then
    begin
...
begin
  ...
  qClienteProdMes.SQL.Add('   AND  (Dbnotafiscal.NumNota = Dbnotafiscalitens.NumNota)');
  ...
  qClienteProdMes.SQL.Add('  AND  ((Dbnotafiscal.NFeDenegada = False) or (Dbnotafiscal.NFeDenegada is Null)) ');
  qClienteProdMes.SQL.Add('  AND ((Dbnotafiscal.CondPgto = ''V'') or (Dbnotafiscal.CondPgto = ''P'') or (Dbnotafiscal.CondPgto = ''A'')) ');
  qClienteProdMes.SQL.Add('  AND (Dbnotafiscal.SaidaEntrada = ''S'') ');
  if not CheckBox4.Checked then
  ...
  if RxDBLookupCombo4.Text <> '' then
    begin
      qClienteProdMes.SQL.Add('  AND ((dbNotaFiscal.CodVendedor = :CodVendedor) or (dbNotaFiscal.CodVendedor2 = :CodVendedor2))');
      qClienteProdMes.ParamByName('CodVendedor').AsInteger  := RxDBLookupCombo4.KeyValue;
      qClienteProdMes.ParamByName('CodVendedor2').AsInteger := RxDBLookupCombo4.KeyValue;
    end;
  if DateEdit1.Date > 0 then
    begin
...
begin
  ...
  if RxDBLookupCombo4.Text <> '' then
    begin
      qPedidoProdMes.SQL.Add('  AND ((dbPedido.CodVendedor = :CodVendedor) or (dbPedido.CodVendedor2 = :CodVendedor2))');
      qPedidoProdMes.ParamByName('CodVendedor').AsInteger  := RxDBLookupCombo4.KeyValue;
      qPedidoProdMes.ParamByName('CodVendedor2').AsInteger := RxDBLookupCombo4.KeyValue;
    end;
  if DateEdit1.Date > 0 then
    begin
...
begin
  ...
  if RxDBLookupCombo4.Text <> '' then
    begin
      qPedidoProdMesCli.SQL.Add('  AND ((dbPedido.CodVendedor = :CodVendedor) or (dbPedido.CodVendedor2 = :CodVendedor2))');
      qPedidoProdMesCli.ParamByName('CodVendedor').AsInteger  := RxDBLookupCombo4.KeyValue;
      qPedidoProdMesCli.ParamByName('CodVendedor2').AsInteger := RxDBLookupCombo4.KeyValue;
    end;
  if DateEdit1.Date > 0 then
    begin
...
begin
  ...
  qProdutoMes.SQL.Add('   AND  (Dbnotafiscal.NumNota = Dbnotafiscalitens.NumNota)');
  if RxDBLookupCombo5.Text <> '' then
    begin
  ...
  qProdutoMes.SQL.Add('  AND ((Dbnotafiscal.NFeDenegada = False) or (Dbnotafiscal.NFeDenegada is Null)) ');
  qProdutoMes.SQL.Add('  AND (Dbnotafiscal.dtemissao  >= :DtInicial) ');
  qProdutoMes.SQL.Add('  AND (Dbnotafiscal.dtemissao  <= :DtFinal) ');
  qProdutoMes.SQL.Add('  AND ((Dbnotafiscal.CondPgto = ''V'') or (Dbnotafiscal.CondPgto = ''P'') or (Dbnotafiscal.CondPgto = ''A'')) ');
  qProdutoMes.SQL.Add('  AND (Dbnotafiscal.SaidaEntrada = ''S'') ');
  if not CheckBox4.Checked then
  ...
  if RxDBLookupCombo4.Text <> '' then
    begin
      qProdutoMes.SQL.Add('  AND ((dbNotaFiscal.CodVendedor = :CodVendedor) or (dbNotaFiscal.CodVendedor2 = :CodVendedor2))');
      qProdutoMes.ParamByName('CodVendedor').AsInteger  := RxDBLookupCombo4.KeyValue;
      qProdutoMes.ParamByName('CodVendedor2').AsInteger := RxDBLookupCombo4.KeyValue;
    end;
  if RxDBLookupCombo3.Text <> '' then
    begin
...
begin
  ...
  qProdutoMesCli.SQL.Add('   AND  (Dbnotafiscal.NumNota = Dbnotafiscalitens.NumNota)');
  ...
  qProdutoMesCli.SQL.Add('  AND ((Dbnotafiscal.NFeDenegada = False) or (Dbnotafiscal.NFeDenegada is Null)) ');
  qProdutoMesCli.SQL.Add('  AND (Dbnotafiscal.dtemissao  >= :DtInicial) ');
  qProdutoMesCli.SQL.Add('  AND (Dbnotafiscal.dtemissao  <= :DtFinal) ');
  qProdutoMesCli.SQL.Add('  AND ((Dbnotafiscal.CondPgto = ''V'') or (Dbnotafiscal.CondPgto = ''P'') or (Dbnotafiscal.CondPgto = ''A'')) ');
  qProdutoMesCli.SQL.Add('  AND (Dbnotafiscal.SaidaEntrada = ''S'') ');
  if not CheckBox4.Checked then
  ...
  if RxDBLookupCombo4.Text <> '' then
    begin
      qProdutoMesCli.SQL.Add('  AND ((dbNotaFiscal.CodVendedor = :CodVendedor) or (dbNotaFiscal.CodVendedor2 = :CodVendedor2))');
      qProdutoMesCli.ParamByName('CodVendedor').AsInteger  := RxDBLookupCombo4.KeyValue;
      qProdutoMesCli.ParamByName('CodVendedor2').AsInteger := RxDBLookupCombo4.KeyValue;
    end;
  if RxDBLookupCombo3.Text <> '' then
    begin
...
begin
  ...
  if RxDBLookupCombo4.Text <> '' then
    begin
      qPedidoProd.SQL.Add('  AND ((dbPedido.CodVendedor = :CodVendedor) or (dbPedido.CodVendedor2 = :CodVendedor2))');
      qPedidoProd.ParamByName('CodVendedor').AsInteger  := RxDBLookupCombo4.KeyValue;
      qPedidoProd.ParamByName('CodVendedor2').AsInteger := RxDBLookupCombo4.KeyValue;
    end;
  if DateEdit1.Date > 0 then
    begin
...
begin
  ...
  if RxDBLookupCombo4.Text <> '' then
    begin
      qPedidoCliMes.SQL.Add('  AND ((dbPedido.CodVendedor = :CodVendedor) or (dbPedido.CodVendedor2 = :CodVendedor2))');
      qPedidoCliMes.ParamByName('CodVendedor').AsInteger  := RxDBLookupCombo4.KeyValue;
      qPedidoCliMes.ParamByName('CodVendedor2').AsInteger := RxDBLookupCombo4.KeyValue;
    end;
  if DateEdit1.Date > 0 then
    begin
...
begin
  ...
  qNotas.SQL.Add('   AND  (Dbnotafiscal.NumNota = Dbnotafiscalitens.NumNota)');
  ...
  if RxDBLookupCombo4.Text <> '' then
    begin
      qNotas.SQL.Add('  AND ((dbNotaFiscal.CodVendedor = :CodVendedor) or (dbNotaFiscal.CodVendedor2 = :CodVendedor2))');
      qNotas.ParamByName('CodVendedor').AsInteger  := RxDBLookupCombo4.KeyValue;
      qNotas.ParamByName('CodVendedor2').AsInteger := RxDBLookupCombo4.KeyValue;
    end;
  if DateEdit1.Date > 0 then
    begin
...
begin
  qVendedor.Close;
  qVendedor.SQL.Clear;
  qVendedor.SQL.Add('SELECT SUM( Dbnotafiscalitens.VlrTotal ) VlrTotal, SUM( Dbnotafiscalitens.Qtd ) Qtd, Dbnotafiscal.PercDesc, Dbnotafiscal.CodVendedor, Dbnotafiscal.CodVendedor2');
  ...
  qVendedor.SQL.Add('   AND  (Dbnotafiscal.NumNota = Dbnotafiscalitens.NumNota)');
  ...
  qVendedor.SQL.Add('  AND ((Dbnotafiscal.NFeDenegada = False) or (Dbnotafiscal.NFeDenegada is Null)) ');
  qVendedor.SQL.Add('  AND ((Dbnotafiscal.CodVendedor > 0) or (Dbnotafiscal.CodVendedor2 > 0)) ');
  qVendedor.SQL.Add('  AND ((Dbnotafiscal.CondPgto = ''V'') or (Dbnotafiscal.CondPgto = ''P'') or (Dbnotafiscal.CondPgto = ''A'')) ');
  qVendedor.SQL.Add('  AND (Dbnotafiscal.SaidaEntrada = ''S'') ');
  if not CheckBox4.Checked then
  ...
  if RxDBLookupCombo4.Text <> '' then
    begin
      qVendedor.SQL.Add('  AND ((dbNotaFiscal.CodVendedor = :CodVendedor) or (dbNotaFiscal.CodVendedor2 = :CodVendedor2))');
      qVendedor.ParamByName('CodVendedor').AsInteger  := RxDBLookupCombo4.KeyValue;
      qVendedor.ParamByName('CodVendedor2').AsInteger := RxDBLookupCombo4.KeyValue;
    end;
  if DateEdit1.Date > 0 then
    begin
  ...
  qVendedor.SQL.Add('GROUP BY Dbnotafiscal.PercDesc, Dbnotafiscal.CodVendedor, Dbnotafiscal.CodVendedor2');
  ...
  while not qVendedor.Eof do
    begin
      ...
      if (qVendedorCodVendedor2.AsInteger > 0) and (qVendedorCodVendedor2.AsInteger <> qVendedorCodVendedor.AsInteger) then
        Grava_mVendedor(qVendedorCodVendedor2.AsInteger);
      qVendedor.Next;
    end;
...
begin
  ...
  qClienteProdCor.SQL.Add('   AND  (Dbnotafiscal.NumNota = Dbnotafiscalitens.NumNota)');
  ...
  qClienteProdCor.SQL.Add('  AND ((Dbnotafiscal.NFeDenegada = False) or (Dbnotafiscal.NFeDenegada is Null)) ');
  qClienteProdCor.SQL.Add('  AND ((Dbnotafiscal.CondPgto = ''V'') or (Dbnotafiscal.CondPgto = ''P'') or (Dbnotafiscal.CondPgto = ''A'')) ');
  qClienteProdCor.SQL.Add('  AND (Dbnotafiscal.SaidaEntrada = ''S'') ');
  if not CheckBox4.Checked then
  ...
  if RxDBLookupCombo4.Text <> '' then
    begin
      qClienteProdCor.SQL.Add('  AND ((dbNotaFiscal.CodVendedor = :CodVendedor) or (dbNotaFiscal.CodVendedor2 = :CodVendedor2))');
      qClienteProdCor.ParamByName('CodVendedor').AsInteger  := RxDBLookupCombo4.KeyValue;
      qClienteProdCor.ParamByName('CodVendedor2').AsInteger := RxDBLookupCombo4.KeyValue;
    end;
  if DateEdit1.Date > 0 then
    begin
...
begin
  ...
  qProduto.SQL.Add('   AND  (Dbnotafiscal.NumNota = Dbnotafiscalitens.NumNota)');
  ...
  qProduto.SQL.Add('  AND ((Dbnotafiscal.NFeDenegada = False) or (Dbnotafiscal.NFeDenegada is Null)) ');
  qProduto.SQL.Add('  AND ((Dbnotafiscal.CondPgto = ''V'') or (Dbnotafiscal.CondPgto = ''P'') or (Dbnotafiscal.CondPgto = ''A'')) ');
  qProduto.SQL.Add('  AND (Dbnotafiscal.SaidaEntrada = ''S'') ');
  if not CheckBox4.Checked then
  ...
  if RxDBLookupCombo4.Text <> '' then
    begin
      qProduto.SQL.Add('  AND ((dbNotaFiscal.CodVendedor = :CodVendedor) or (dbNotaFiscal.CodVendedor2 = :CodVendedor2))');
...
begin
  ...
  qClienteMes.SQL.Add('   AND (Dbnotafiscal.NumNota = Dbnotafiscalitens.NumNota)');
  ...
  qClienteMes.SQL.Add('  AND ((Dbnotafiscal.NFeDenegada = False) or (Dbnotafiscal.NFeDenegada is Null)) ');
  qClienteMes.SQL.Add('  AND (Dbnotafiscal.dtemissao  >= :DtInicial) ');
  qClienteMes.SQL.Add('  AND (Dbnotafiscal.dtemissao  <= :DtFinal) ');
  qClienteMes.SQL.Add('  AND ((Dbnotafiscal.CondPgto = ''V'') or (Dbnotafiscal.CondPgto = ''P'') or (Dbnotafiscal.CondPgto = ''A'')) ');
  qClienteMes.SQL.Add('  AND (Dbnotafiscal.SaidaEntrada = ''S'') ');
  if not CheckBox4.Checked then
  ...
  if RxDBLookupCombo4.Text <> '' then
    begin
      qClienteMes.SQL.Add('  AND ((dbNotaFiscal.CodVendedor = :CodVendedor) or (dbNotaFiscal.CodVendedor2 = :CodVendedor2))');
      qClienteMes.ParamByName('CodVendedor').AsInteger  := RxDBLookupCombo4.KeyValue;
      qClienteMes.ParamByName('CodVendedor2').AsInteger := RxDBLookupCombo4.KeyValue;
    end;
  if RxDBLookupCombo3.Text <> '' then
    begin
...
begin
  ...
  qClienteProd.SQL.Add('   AND  (Dbnotafiscal.NumNota = Dbnotafiscalitens.NumNota)');
  ...
  qClienteProd.SQL.Add('  AND  ((Dbnotafiscal.NFeDenegada = False) or (Dbnotafiscal.NFeDenegada is Null)) ');
  qClienteProd.SQL.Add('  AND ((Dbnotafiscal.CondPgto = ''V'') or (Dbnotafiscal.CondPgto = ''P'') or (Dbnotafiscal.CondPgto = ''A'')) ');
  qClienteProd.SQL.Add('  AND (Dbnotafiscal.SaidaEntrada = ''S'') ');
  if not CheckBox4.Checked then             
  ...
  if RxDBLookupCombo4.Text <> '' then
    begin
      qClienteProd.SQL.Add('  AND ((dbNotaFiscal.CodVendedor = :CodVendedor) or (dbNotaFiscal.CodVendedor2 = :CodVendedor2))');
      qClienteProd.ParamByName('CodVendedor').AsInteger  := RxDBLookupCombo4.KeyValue;
      qClienteProd.ParamByName('CodVendedor2').AsInteger := RxDBLookupCombo4.KeyValue;
    end;
  if DateEdit1.Date > 0 then
    begin
  ...
  mClienteProd.EmptyDataSet;
  ProgressBar2.Max      := qClienteProd.RecordCount + qPedidoCliProd.RecordCount + qDevolucao2.RecordCount;
  ...
  //Monta as devoluções
  qDevolucao2.First;
  while not qDevolucao2.Eof do
    begin
      ProgressBar2.Position := ProgressBar2.Position + 1;
      Monta_CodAgrupadoDev;
      if vGera then
        begin
          if mClienteProd.Locate('CodAgrupado;CodProduto',VarArrayOf([vCodAgrupado,qDevolucao2.FieldByName('CodProduto').AsInteger]),[locaseinsensitive]) then
            mClienteProd.Edit
          else
            begin
              mClienteProd.Insert;
              mClienteProdCodAgrupado.AsString := vCodAgrupado;
              mClienteProdCodCliente.AsInteger := DM1.tClienteCodigo.AsInteger;
              mClienteProdCodProduto.AsInteger := qDevolucao2.FieldByName('CodProduto').AsInteger;
              mClienteProdReferencia.AsString  := qDevolucao2.FieldByName('Referencia').AsString;
              mClienteProdNomeProduto.AsString := qDevolucao2.FieldByName('NomeProduto').AsString;
              mClienteProdCodGrupo.AsInteger   := DM1.tClienteCodGrupo.AsInteger;
              if copy(vCodAgrupado,1,1) = 'C' then
                begin
                  mClienteProdNomeCliente.AsString := DM1.tClienteNome.AsString;
                  mClienteProdFantasia.AsString    := DM1.tClienteFantasia.AsString;
                end
              else
              if copy(vCodAgrupado,1,1) = 'G' then
                begin
                  mClienteProdFantasia.AsString    := DM1.tClientelkNomeGrupo.AsString;
                  mClienteProdNomeCliente.AsString := DM1.tClientelkNomeGrupo.AsString;
                end
              else
              if copy(vCodAgrupado,1,1) = 'F' then
                begin
                  mClienteProdCodCliente.AsInteger := qDevolucao2.FieldByName('CodForn').AsInteger;
                  mClienteProdFantasia.AsString    := qDevolucao2.FieldByName('NomeForn').AsString;
                  mClienteProdNomeCliente.AsString := qDevolucao2.FieldByName('NomeForn').AsString;
                  mClienteProdCodGrupo.AsInteger   := 0;
                end;
            end;
          mClienteProdVlrTotal.AsFloat := mClienteProdVlrTotal.AsFloat - StrToFloat(FormatFloat('0.00',qDevolucao2.FieldByName('VlrDevolucao').AsFloat));
          mClienteProdQtd.AsFloat      := mClienteProdQtd.AsFloat - qDevolucao2.FieldByName('Qtd').AsFloat;
          if mClienteProdVlrTotal.AsFloat < 0 then
            begin
              mClienteProdVlrTotal.AsFloat := 0;
              mClienteProdQtd.AsFloat      := 0;
            end;
          if mClienteProdVlrTotal.AsFloat > 0 then
            mClienteProdPercFat.AsFloat  := StrToFloat(FormatFloat('0.00000',(mClienteProdVlrTotal.AsFloat / CurrencyEdit3.Value) * 100));
          mClienteProdVlrDevolucao.AsFloat := mClienteProdVlrDevolucao.AsFloat + StrToFloat(FormatFloat('0.00',qDevolucao2.FieldByName('VlrDevolucao').AsFloat));
          mClienteProd.Post;
        end;
      qDevolucao2.Next;
    end;
...
begin
  ...
  qFaturamento.SQL.Add('  AND  ((Dbnotafiscal.NFeDenegada = False) or (Dbnotafiscal.NFeDenegada is Null)) ');
  qFaturamento.SQL.Add('  AND  ((Dbnotafiscal.CondPgto = ''V'') or (Dbnotafiscal.CondPgto = ''P'') or (Dbnotafiscal.CondPgto = ''A'')) ');
  qFaturamento.SQL.Add('  AND (Dbnotafiscal.SaidaEntrada = ''S'') ');
  if not CheckBox4.Checked then
...
procedure TfHistGeral.Monta_SQLDevolucao;
var
  vVlrDev : Real;
  ano, mes, dia : Word;
  vDataAux : String;
begin
  mDevMensal.EmptyDataSet;

  qDevolucao.Close;
  qDevolucao.SQL.Clear;
  qDevolucao.SQL.Add('SELECT Dbnentrada.DtEntrada, SUM( Dbnentrada.VlrDevolucao ) VlrDevolucao');
  qDevolucao.SQL.Add('FROM dbnentrada Dbnentrada');
  qDevolucao.SQL.Add('   INNER JOIN "dbNEntradaItens.DB" Dbnentradaitens');
  qDevolucao.SQL.Add('   ON  (Dbnentrada.Filial = Dbnentradaitens.Filial)');
  qDevolucao.SQL.Add('   AND  (Dbnentrada.NumNEntr = Dbnentradaitens.NumNEntr)');
  qDevolucao.SQL.Add('   AND  (Dbnentrada.CodForn = Dbnentradaitens.CodForn)');
  qDevolucao.SQL.Add('WHERE  Dbnentrada.VlrDevolucao > 0');
  if not CheckBox4.Checked then
    qDevolucao.SQL.Add('  AND  (dbNEntradaItens.MatProd = ''P'')');
  if RxDBLookupCombo2.Text <> '' then
    begin
      qDevolucao.SQL.Add('  AND (dbNEntrada.Filial = :Filial)');
      qDevolucao.ParamByName('Filial').AsInteger := RxDBLookupCombo2.KeyValue;
    end;
  if DateEdit1.Date > 0 then
    begin
      qDevolucao.SQL.Add('  AND (Dbnentrada.DtEntrada >= :DtEntradaIni)');
      qDevolucao.ParamByName('DtEntradaIni').AsDate := DateEdit1.Date;
    end;
  if DateEdit2.Date > 0 then
    begin
      qDevolucao.SQL.Add('  AND (Dbnentrada.DtEntrada <= :DtEntradaFin)');
      qDevolucao.ParamByName('DtEntradaFin').AsDate := DateEdit2.Date;
    end;
  qDevolucao.SQL.Add('GROUP BY Dbnentrada.DtEntrada');
  qDevolucao.Open;

  vVlrDev := 0;
  qDevolucao.First;
  while not qDevolucao.Eof do
    begin
      vVlrDev := vVlrDev + StrToFloat(FormatFloat('0.00',qDevolucaoVlrDevolucao.AsFloat));
      DecodeDate(qDevolucaoDtEntrada.AsDateTime,ano,mes,dia);
      vDataAux := FormatFloat('00',mes) + '/' + FormatFloat('0000',ano);
      if mDevMensal.Locate('Data',vDataAux,[loCaseInsensitive]) then
        mDevMensal.Edit
      else
        begin
          mDevMensal.Insert;
          mDevMensalData.AsString := vDataAux;
        end;
      mDevMensalVlrDevolucao.AsFloat := mDevMensalVlrDevolucao.AsFloat + StrToFloat(FormatFloat('0.00',qDevolucaoVlrDevolucao.AsFloat));
      qDevolucao.Next;
    end;
  CurrencyEdit2.Value := vVlrDev;
...
begin
  ...
  qCliente.SQL.Add('  AND ((Dbnotafiscal.CondPgto = ''V'') or (Dbnotafiscal.CondPgto = ''P'') or (Dbnotafiscal.CondPgto = ''A'')) ');
  ...
  if RxDBLookupCombo4.Text <> '' then
    begin
      qCliente.SQL.Add('  AND ((dbNotaFiscal.CodVendedor = :CodVendedor) or (dbNotaFiscal.CodVendedor2 = :CodVendedor2))');
      qCliente.ParamByName('CodVendedor').AsInteger  := RxDBLookupCombo4.KeyValue;
      qCliente.ParamByName('CodVendedor2').AsInteger := RxDBLookupCombo4.KeyValue;
    end;
  if DateEdit1.Date > 0 then
    begin
  ...
  mCliente.EmptyDataSet;
  ProgressBar3.Max      := qCliente.RecordCount + qDevolucao2.RecordCount;
  ...
  //Monta o auxiliar lendo a sql da devolução
  qDevolucao2.First;
  while not qDevolucao2.Eof do
    begin
      ProgressBar3.Position := ProgressBar3.Position + 1;
      ...
      if vGera then
        begin
          if mCliente.Locate('CodAgrupado',vCodAgrupado,[loCaseInsensitive]) then
            ...
            begin
              mCliente.Insert;
              mClienteCodCliente.AsInteger := DM1.tClienteCodigo.AsInteger;
              if copy(vCodAgrupado,1,1) = 'C' then
                begin
                  mClienteNomeCliente.AsString := DM1.tClienteNome.AsString;
                  mClienteFantasia.AsString    := DM1.tClienteFantasia.AsString;
                end
              else
              if copy(vCodAgrupado,1,1) = 'G' then
                begin
                  mClienteFantasia.AsString    := DM1.tClientelkNomeGrupo.AsString;
                  mClienteNomeCliente.AsString := DM1.tClientelkNomeGrupo.AsString;
                end
              else
              if copy(vCodAgrupado,1,1) = 'F' then
                begin
                  mClienteCodCliente.AsInteger := qDevolucao2.FieldByName('CodForn').AsInteger;
                  mClienteFantasia.AsString    := qDevolucao2.FieldByName('NomeForn').AsString;
                  mClienteNomeCliente.AsString := qDevolucao2.FieldByName('NomeForn').AsString;
                end;
              mClienteCodAgrupado.AsString := vCodAgrupado;
            end;
          mClienteVlrDevolucao.AsFloat  := mClienteVlrDevolucao.AsFloat + StrToFloat(FormatFloat('0.00',qDevolucao2.FieldByName('VlrDevolucao').AsFloat));
          mClienteVlrTotal.AsFloat      := mClienteVlrTotal.AsFloat - StrToFloat(FormatFloat('0.00',qDevolucao2.FieldByName('VlrDevolucao').AsFloat));
          mClientePercFat.AsFloat       := StrToFloat(FormatFloat('0.00000',(mClienteVlrTotal.AsFloat / CurrencyEdit3.Value) * 100));
          mClienteQtd.AsFloat           := mClienteQtd.AsFloat - qDevolucao2.FieldByName('Qtd').AsFloat;
          mCliente.Post;
        end;
      qDevolucao2.Next;
    end;
...
procedure TfHistGeral.RxDBLookupCombo5Enter(Sender: TObject);
begin
