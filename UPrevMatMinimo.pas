...
begin
  ...
  qEstoque.Close;
  qEstoque.SQL.Clear;
  qEstoque.SQL.Add('SELECT Dbestoquematmov.CodMaterial, Dbestoquematmov.CodCor, Dbproduto.EstMinimo, Dbproduto.Nome NomeMaterial, Dbproduto.Referencia, Dbcor.Nome NomeCor, SUM( Dbestoquematmov.Qtd2 ) Saldo, Dbestoquematmov.Filial');
  ...
  qEstoque.SQL.Add('GROUP BY Dbestoquematmov.CodMaterial, Dbestoquematmov.CodCor, Dbproduto.EstMinimo, Dbproduto.Nome, Dbproduto.Referencia, Dbcor.Nome, Dbestoquematmov.Filial');
  qEstoque.SQL.Add('ORDER BY NomeMaterial, NomeCor');
  qEstoque.Open;
  qEstoque.Filtered := False;
  qEstoque.Filter   := 'Filial = '''+IntToStr(RxDBLookupCombo3.KeyValue)+'''';
