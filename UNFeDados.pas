...
uses Variants, uNFe, UDMNotaFiscal, UDMNFe, uUtil_NFe;
...
begin
  ...
  while not DM1.tNotaFiscalRef.Eof do
    begin
      ...
      begin
        vObsAux := vObsAux + '(NF referenciada de Produto Rural Mes/Ano: ' + DM1.tNotaFiscalRefMesEmissaoRef.AsString + '/' + DM1.tNotaFiscalRefAnoEmissaoRef.AsString
                   + ', Modelo ' + DM1.tNotaFiscalRefModeloRef.AsString + ', Serie ' + DM1.tNotaFiscalRefSerieRef.AsString
                   + ', Nº ' + DM1.tNotaFiscalRefNumNotaRef.AsString + ', CNPJ/CPF: ' + DM1.tNotaFiscalRefCNPJEmitenteRef.AsString
                   + ', UF: ' + DM1.tNotaFiscalRefCodUFEmitenteRef.AsString + ', IE: ' + DM1.tNotaFiscalRefIE.AsString + ')';
      end
      else
...
begin
  ...
  while not fNFe.mAuxDadosNFe.Eof do
    begin
      ...
      begin
        ...
        begin
          if Trim(DM1.tNatOperacaoLei.Value) <> '' then
  ...
  begin
    ...
    if DM1.tNotaFiscallkPessoaPgto.AsString = 'J' then
      vTexto := vTexto + ' - CNPJ: ' + DM1.tNotaFiscallkCgcCpfPgto.AsString
    else
    if DM1.tNotaFiscallkPessoaPgto.AsString = 'F' then
      vTexto := vTexto + ' - CPF: ' + DM1.tNotaFiscallkCgcCpfPgto.AsString;
    vTexto := vTexto + ')';
    Grava_DadosAdicionaisNFe(vTexto,0);
  end;
  ...
  begin
    ...
    if DM1.tNotaFiscallkPessoaEntrega.AsString = 'J' then
      vObsAux := vObsAux + ' - CNPJ: ' + DM1.tNotaFiscallkCgcCpfEntrega.AsString
    else
    if DM1.tNotaFiscallkPessoaEntrega.AsString = 'F' then
      vObsAux := vObsAux + ' - CPF: ' + DM1.tNotaFiscallkCgcCpfEntrega.AsString;
    vObsAux := vObsAux + ')';
    Grava_DadosAdicionaisNFe(vObsAux,0);
  end;
  ...
  while not DM1.tNotaFiscalParc.Eof do
  begin
    if (DM1.tNotaFiscalParclkDeposito.AsBoolean) and (DM1.tNotaFiscalParcCodConta.AsInteger > 0) then
    begin
      vTexto := '(Deposito: Age: ' + DM1.tNotaFiscalParclkAgencia.AsString
              + ', Conta: ' + DM1.tNotaFiscalParclkNumConta.AsString + DM1.tNotaFiscalParclkDigConta.AsString
              + ', ' + DM1.tNotaFiscalParclkDescNota.AsString
              + ')';
...
begin
  ...
  if DM1.tProduto.GotoKey then
    begin
      if DM1.tNotaFiscalItensCodGrade.AsInteger > 0 then
        begin
          if DM1.tProdutoCli.Locate('CodCor;CodCliente;CodGrade;Posicao',
             VarArrayOf([DM1.tNotaFiscalItensCodCor.AsInteger,DM1.tNotaFiscalCodCli.AsInteger,DM1.tNotaFiscalItensCodGrade.AsInteger,DM1.tNotaFiscalItensPosicao.AsInteger]),[locaseinsensitive]) then
            vCodProdutoNFe := DM1.tProdutoCliCodProdCli.AsString
          else
          if DM1.tProdutoCli.Locate('CodCor;CodCliente;CodGrade',
             VarArrayOf([DM1.tNotaFiscalItensCodCor.AsInteger,DM1.tNotaFiscalCodCli.AsInteger,DM1.tNotaFiscalItensCodGrade.AsInteger]),[locaseinsensitive]) then
            vCodProdutoNFe := DM1.tProdutoCliCodProdCli.AsString
        end;
...
begin
  ...
  while not DM1.tNotaFiscalItens.Eof do
    begin
      ...
      if (Trim(DM1.tNotaFiscalItensClasFiscal.AsString) = '') and (Copy(fNFe.mItensNFeCodProduto.AsString,1,4) = 'CFOP') then
        fNFe.mItensNFeClasFiscal.AsString    := '99';
      ...
      fNFe.mItensNFeIcmsBaseSubst.AsString := DM1.tNotaFiscalItensIcmsBaseSubst.AsString;
      fNFe.mItensNFeIcmsVlrSubst.AsString  := DM1.tNotaFiscalItensIcmsVlrSubst.AsString;
