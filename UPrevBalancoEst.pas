...
uses
  ...
  DBTables, Mask, ToolEdit, ComCtrls;
...
type
  TfPrevBalancoEst = class(TForm)
    ...
    qEstoqueCustoFilial: TIntegerField;
    CheckBox1: TCheckBox;
    Label5: TLabel;
    ComboBox1: TComboBox;
...
begin
  ...
  if RxDBLookupCombo1.Text <> '' then
    qEstoqueCusto.Filter   := 'CodMaterial = '''+DM1.tProdutoCodigo.AsString+''' and DtMov <='''+DateEdit1.Text+''' and Filial = '''+IntToStr(RxDBLookupCombo1.KeyValue)+''''
  else
...
begin
  ...
  while not DM1.tProduto.Eof do
    begin
      if (DM1.tProdutoInativo.AsBoolean = False) or (CheckBox1.Checked) then
        begin
          if (DM1.tProdutoEstoque.AsBoolean) and (DM1.tProdutoProdMat.AsString = vProdMat) then
            begin
