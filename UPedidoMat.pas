...
type
  TfPedidoMat = class(TForm)
    ...
    mPedidoConsumoTamanhoMat: TStringField;
    BitBtn2: TBitBtn;
        ...
        1 : begin
              ...
              while not mPedidoConsumo.Eof do
                begin
                  ...
                  DM1.tPedidoConsumoTamanhoMat.AsString   := mPedidoConsumoTamanhoMat.AsString;
...
begin
  if DM1.tPedidoMaterialOrdem.AsInteger > 0 then
    begin
      vItem := DM1.tPedidoMaterialOrdem.AsInteger;
        ...
        begin
          ...
          while not DM1.tPedidoConsumo.Eof do
            begin
              ...
              mPedidoConsumoTamanhoMat.AsString := DM1.tPedidoConsumoTamanhoMat.AsString;
              mPedidoConsumo.Post;
              DM1.tPedidoConsumo.Delete;
            end;
          mPedidoConsumo.First;
...
procedure TfPedidoMat.GroupBox1Enter(Sender: TObject);
begin
  if DM1.tPedidoConsumo.RecordCount > 0 then
    VDBGrid1.Columns[2].ReadOnly := not(DM1.tPedidoMateriallkFichaTecnicaConsPorTam.AsBoolean)
  else
    RxDBGrid1.SetFocus;
end;
...
begin
  if MessageDlg('                                  ATENÇÃO'
      +#13#13+
      'Se confirmar a atualização dos materiais para este item, o sistema ira excluir'+#13+
      ' os materiais existentes no item e vai gravar os materiais que estão no produto'+#13+
      '  Clique no botão "Não" para sair sem atualizar.',mtInformation,
      [mbOK,mbNO],0) = mrOK then
    begin
      RxDBFilter1.Active := False;
      fPedido.Atualiza_Materiais;
      RxDBFilter1.Active := True;
    end;
end;
