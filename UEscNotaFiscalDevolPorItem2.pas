...
begin
  ...
  qMaterial.SQL.Add('   AND  (Dbnotafiscal.NumNota = Dbnotafiscalitens.NumNota)');
  ...
  qMaterial.SQL.Add('      AND  (DbNotaFiscal.CodCli = :CodCli)');
...
begin
  ...
  qNotaFiscal.SQL.Add('SELECT Dbnotafiscal.Filial, Dbnotafiscal.NumNota, Dbnotafiscal.DtEmissao, Dbnotafiscalitens.CodProduto, Dbproduto.Referencia, Dbnotafiscal.CodCli, ');
  ...
  qNotaFiscal.SQL.Add(' Dbnotafiscalitens.QtdDevolvida, Dbnotafiscalitens.CodCor, Dbnotafiscalitens.Unidade, Dbnatoperacao.CodNatOper, Dbnotafiscalitens.VlrUnit, Dbnotafiscalitens.VlrTotal, dbNotaFiscal.NumSeq');
  ...
  qNotaFiscal.ParamByName('Filial').AsInteger := DM1.tFilialCodigo.AsInteger;
...
begin
  ...
  if vGravar then
    begin
      ...
      fNotaEntrada.RZDBLookupComboBox1.KeyValue := fNotaEntrada.mSelecionadoCodMaterial.AsInteger;
      fNotaEntrada.Panel6Exit(Sender);

      fNotaEntrada.RxDBLookupCombo2.KeyValue := fNotaEntrada.mSelecionadoCodCor.AsInteger;
      fNotaEntrada.RxDBLookupCombo15.Value   := fNotaEntrada.mSelecionadoUnidade.AsString;
...
begin
  ...
  if DM1.tCliente.Locate('CgcCpf',DM1.tFornecedoresCGC.AsString,[loCaseInsensitive]) then
    ...
    ShowMessage('Cliente não encontrado com o cgc do fornecedor!');
  qMaterial.Close;
